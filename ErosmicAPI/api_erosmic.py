...
JELLYFIN_INTERNAL_URL = os.environ.get("JELLYFIN_INTERNAL_URL")
...
def build_track(item):  # if the items provided does not have the information, its listed as unknown.
    album_id = item.get("AlbumId")  # Jellyfin's album ID, used for album art
    return {
        "id": item.get("Id"),
        ...
        # StreamUrl sets the URL from the audio
        "streamUrl":
...
async def get_tracks():  # Allows the request coming from FastAPI to run in a asynchronous time.
    try:
            ...
            response = await client.get(
                ...
                params={
                    ...
                    "Fields":
                        ...
                        "Genres,"
                        "RunTimeTicks,"
                        "AlbumId",  # added so albumArtUrl can use the album's image
                },
            )

        print("JELLYFIN STATUS:", response.status_code)  # provides status code once executed.
        ...
        items = response.json().get("Items", [])  # Jellyfin responds in JSON to the list in "items", [] if there are no items.
